...
class AudioCapture:
    ...
    def _resolve_device(self) -> LoopbackDeviceInfo:
    ...
    def start(self) -> None:
        ...
        try:
            ...
            self._pa.terminate()
            self._pa = None
            raise AudioCaptureError(f"Failed to open loopback stream on '{info.name}': {e}") from e

        self._running = True
        logger.info(
            "Audio capture started: device='%s' rate=%d ch=%d block=%d",
            self.device_name,
            self.samplerate,
            self.channels,
            self.block_size,
